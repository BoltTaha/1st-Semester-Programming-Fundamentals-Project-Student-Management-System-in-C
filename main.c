...
void student_information() {
    ...
    do {
        ...
        printf("3. Press 3 to enter the main menu again\n");
        
        if (scanf("%d", &flag) != 1) {
            printf("Input failed or is not a valid number.\n");
            while (getchar() != '\n');
    ...
    if (flag == 1) {
        ...
        if (ch >= 1 && ch <= 5) {
            char filename[100];
            sprintf(filename, "class%d.txt", ch);
...
do {
     ...
     save_to_class_file(student.class_num, student.first_name, student.last_name, student.rollno);
        ...
        save_student_info_file(student);
          // passing value to function to save data in All student records file
